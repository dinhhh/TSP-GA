...
def mutate(individual, mutateRate):
	for swapped in range(1, len(individual.chromosome)):
...
def nextGeneration(population, selectionSize, elitism, mutateRate):
	...
	nextGene = mutatedPopulation(crossoverPopu, mutateRate)
	return nextGene
...
def algorithm(population, selectionSize, elitism, mutateRate, loops):
	...
	for i in range (loops):
		...
		sortedList = sorted(popu, key= lambda x: x.fitness, reverse=True)
		#printFitnessPopulation(sortedList)
		sortedList[0].printFitness()
		print("LOOP TIME: " + str(i))
	return popu
...
def main():
	...
	popu = initPopulation(cityList, 5, cityList[0])
